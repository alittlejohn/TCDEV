...
function restlet(request, response) {
	...
	try {
		...
		if (e instanceof nlobjError) {
			nlapiSendEmail(3, ['sharon.mylan@gwfathom.com,dev@truecloud.com','fathom-it@gwfathom.com'], 'Global Water Fathom Integration : '+(data.refund != 'true' ? 'Journal Entry' : 'Refunds')+' FAILURE', 'Stack Trace: '+e.getStackTrace()+'<br><br>'+e.getDetails()+'<br><br>File Name: '+data.datafilename);
			error.code = e.getCode(), error.details = e.getDetails();
		} else {
			nlapiSendEmail(3, ['sharon.mylan@gwfathom.com,dev@truecloud.com','fathom-it@gwfathom.com'], 'Global Water Fathom Integration : '+(data.refund != 'true' ? 'Journal Entry' : 'Refunds')+' FAILURE', JSON.stringify(e));
