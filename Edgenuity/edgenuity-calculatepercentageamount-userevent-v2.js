...
function _get_itemTypes() {
	var data = {};
	var filter = [];
		filter.push(new nlobjSearchFilter('isinactive', null, 'is', 'F'));
	var column = [];
		column.push(new nlobjSearchColumn('name'));
	var results = nlapiSearchRecord('customlist113', null, filter, column) || [];
	for (var i = 0, count = results.length ; i < count ; i++) {
		data[results[i].getId()] = results[i].getValue('name');
	}
	nlapiGetContext().setSessionObject('custpage_calc_itemtypes', JSON.stringify(data));
}
...
function before_load(type, form, request) {
	if (type == 'create' || type == 'edit' || type == 'copy') {
		...
		if (valid_forms.indexOf(parseInt(nlapiGetFieldValue('customform'), 10)) != -1) {
			...
			if (!nlapiGetContext().getSessionObject('custpage_calc_itemtypes')) _get_itemTypes();
		}
	}
}