...
/* Valid forms to apply this development too
	...
	*/
	...
	log('Valid Form to apply Percentage Based Calcs?',
		...
		'fieldsexposed': nlapiGetLineItemField('item', 'custcol_item_type') !== null && nlapiGetLineItemField('item', 'custcol_typeofitempercentages') !== null && nlapiGetLineItemField('item', 'custcol_businessamountpercent') !== null
	});
	var validform = valid_forms.indexOf(parseInt(nlapiGetFieldValue('customform'), 10)) != -1 && nlapiGetLineItemField('item', 'custcol_item_type') !== null && nlapiGetLineItemField('item', 'custcol_typeofitempercentages') !== null && nlapiGetLineItemField('item', 'custcol_businessamountpercent') !== null;
...
function _calc_percentage_amount(classes, percentage, line) {
	...
	for (var i = 1, count = nlapiGetLineItemCount('item') ; i <= count ; i++) {
		...
		var current_class = (nlapiGetLineItemValue('item', 'custcol_item_type', i) || '').toString();
	...
	if (line) {
		...
		if (amount === 0) {
			...
			var class_names = JSON.parse(nlapiGetContext().getSessionObject('custpage_calc_itemtypes') || '{}');
			var alert_message = nlapiGetLineItemText('item', 'item', line)+' must be used in conjunction with items that reference these Item Types:\n\n';
...
function _set_business_type_list(type, name) {
	if (type == 'item' && name == 'item' && validform) {
		...
		if (item) {
			var business_types = nlapiLookupField('item', item, 'custitem_typeofitempercentage');
			log('business_types', business_types);
			if (business_types) {
				business_types = business_types.split(',');
				nlapiSetCurrentLineItemValue('item', 'custcol_typeofitempercentages', JSON.stringify(business_types));
			} else {
				nlapiSetCurrentLineItemValue('item', 'custcol_typeofitempercentages', '');
			}
		}
	}
}
...
function _set_percentage_amount() {
	...
	if (nlapiGetCurrentLineItemValue('item', 'custcol_typeofitempercentages')) {
		var classes = JSON.parse(nlapiGetCurrentLineItemValue('item', 'custcol_typeofitempercentages'));
...
function _validate_percentages() {
	...
	for (var i = 1, count = nlapiGetLineItemCount('item') ; i <= count ; i++) {
		var classes = nlapiGetLineItemValue('item', 'custcol_typeofitempercentages', i);
		if (!classes) continue;
